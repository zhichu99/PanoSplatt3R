...
class RoPE2D(torch.nn.Module):
    ...
    def apply_pano_rope1d(self, tokens, pos1d, cos, sin, h, w):
        assert pos1d.ndim==2 and h is not None and w is not None
        nhead = tokens.size(1)
        updated_tokens = []
        for i in range(nhead):
            ...
            head_sin = torch.nn.functional.embedding(pano_pos1d, sin)
            
            updated_token = (tokens[:, i] * head_cos) + (self.rotate_half(tokens[:, i]) * head_sin)
            updated_tokens.append(updated_token)
        
        # Stack all updated heads and return
        tokens = torch.stack(updated_tokens, dim=1)
        return tokens
        
    def forward(self, tokens, positions, pano_rot=False, h=None, w=None):
        """
        input:
        ...
        if pano_rot:
            x = self.apply_pano_rope1d(x, positions[:,:,1], cos, sin, h, w)
        else:
...
class FloatRoPE2D(torch.nn.Module):
    def __init__(self, freq=100.0):
        super(FloatRoPE2D, self).__init__()
        self.base = freq
    ...
    def apply_rope1d(self, tokens, pos1d):
        ...
        inv_freq = 1 / (self.base ** (torch.arange(0, D, 2).float().to(device) / D))
        inv_freq = rearrange(inv_freq, 'c -> 1 c')

        print(inv_freq)
        print('pos1d ', pos1d)
        # t = torch.arange(self.max_length, device=device, dtype=dtype)
        freqs = torch.einsum("b i, b j -> b ij", pos1d, inv_freq).to(dtype)
        freqs = torch.cat((freqs, freqs), dim=-1)
        cos = rearrange(freqs.cos(), 'b t c -> b 1 t c')
        sin = rearrange(freqs.sin(), 'b t c -> b 1 t c')
        print('cos ', cos)
        print('sin ', sin)
...
if __name__ == '__main__':
    ...
    x = repeat(x, 'b h 1 c -> b h 2 c')
    p = torch.tensor([[0,0], [0, 2*np.pi-0.0001]])
    p = rearrange(p, 'n c -> 1 n c')
