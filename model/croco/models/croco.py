...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.effi_attention = effi_attention
        ...
        self.enc_blocks = nn.ModuleList([
            Block(enc_embed_dim, enc_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, rope=self.rope, pano_rot=self.pano_rot, effi_attention=self.effi_attention)
    ...
    def _set_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec):
        ...
        self.dec_blocks = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, norm_mem=norm_im2_in_dec, rope=self.rope, 
                         pano_rot=self.pano_rot, effi_attention=self.effi_attention)
    ...
    def _encode_image(self, image, do_mask=False, return_all_blocks=False, mode='train'):
        """
    ...
    def _decoder(self, feat1, pos1, masks1, feat2, pos2, return_all_blocks=False, h=16, w=32, mode='train'):
        """
        return_all_blocks: if True, return the features at the end of every block 
                           instead of just the features from the last block (eg for some prediction heads)
        ...
        if return_all_blocks:
            ...
            for blk in self.dec_blocks:
                _out, out2 = blk(_out, out2, pos1, pos2, dec_id, h, w, mode)
                out.append(_out)
                dec_id += 1
            out[-1] = self.dec_norm(out[-1])
        else:
            for blk in self.dec_blocks:
                out, out2 = blk(out, out2, pos1, pos2, dec_id, h, w, mode)
                dec_id += 1
    ...
    def forward(self, img1, img2, mode):
        ...
        feat1, pos1, mask1 = self._encode_image(img1, do_mask=True, mode=mode)
        # encoder of the second image 
        feat2, pos2, _ = self._encode_image(img2, do_mask=False, mode=mode)
        # decoder 
        decfeat = self._decoder(feat1, pos1, mask1, feat2, pos2, h, w, mode=mode)
        # prediction head 
