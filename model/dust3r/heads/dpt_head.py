...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def forward(self, x, img_info, ray_embedding=None):
        out, head_feature = self.dpt(x, image_size=(img_info[0], img_info[1]), ray_embedding=ray_embedding)
        if self.postprocess:
            out = self.postprocess(out, self.depth_mode, self.conf_mode)
        return out


def create_dpt_head(net, has_conf=False, out_nchan=3, postprocess_func=postprocess, circular_pad=False):
    """
