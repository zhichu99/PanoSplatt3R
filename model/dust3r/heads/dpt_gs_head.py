...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def forward(self, x, imgs, img_info, conf=None):
        out, head_feature = self.dpt(x, imgs, image_size=(img_info[0], img_info[1]), conf=conf)
        if self.postprocess:
            out = self.postprocess(out, self.depth_mode, self.conf_mode)
        return out


def create_gs_dpt_head(net, has_conf=False, out_nchan=3, postprocess_func=postprocess, upsample=False, circular_pad=False, upsample_factor=None):
    """
