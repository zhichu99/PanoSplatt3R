...
class ViewSamplerBounded(ViewSampler[ViewSamplerBoundedCfg]):
    ...
    def sample(
        self,
        scene: str,
        extrinsics: Float[Tensor, "view v 4 4"],
        intrinsics: Float[Tensor, "view v 3 3"],
        device: torch.device = torch.device("cpu"),
    ) -> tuple[
        Int64[Tensor, " context_view"],  # indices for context views
        Int64[Tensor, " target_view"],  # indices for target views
    ]:
        num_views, _, _, _ = extrinsics.shape
        ...
        if context_gap >= num_views:
            if num_views > min_gap:
                context_gap = torch.randint(
                    min_gap,
                    num_views,
                    size=tuple(),
