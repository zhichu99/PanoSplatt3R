...
def build_imgs_info(database, ref_ids, render_cubes=False, pad_interval=-1, is_aligned=True, align_depth_range=False, has_depth=False, replace_none_depth = False, debug=False):
    ...
    # intrinsics = np.asarray([database.get_K(ref_id) for ref_id in ref_ids], dtype=np.float32)
    ...
    near = np.asarray([database.get_depth_near(ref_id) for ref_id in ref_ids], dtype=np.float32)
    far = np.asarray([database.get_depth_far(ref_id) for ref_id in ref_ids], dtype=np.float32)
    scale = np.array([1.0])
    # ref_poses = np.asarray([database.get_pose(ref_id) for ref_id in ref_ids], dtype=np.float32)
    ...
    pano_mask = np.sum(ref_imgs, axis=1) != 0

    if render_cubes:
        cube_img = np.asarray([database.get_cube_image(ref_id) for ref_id in ref_ids]).transpose([0, 1, 4, 2, 3])
        cube_mask = np.sum(cube_img, axis=2) != 0
        cube_depth = np.asarray([database.get_cube_depth(ref_id) for ref_id in ref_ids], dtype=np.float32)[:, :, None, :, :]
        cube_K = np.asarray([database.get_K(ref_id) for ref_id in ref_ids], dtype=np.float32)
        cube_K = cube_K[:, None, :, :].repeat(5, axis=1).repeat(6, axis=2)
        cube_poses_c2w = np.asarray([database.get_cube_c2w(ref_id) for ref_id in ref_ids], dtype=np.float32)
        cube_poses_w2c = np.asarray([database.get_cube_w2c(ref_id) for ref_id in ref_ids], dtype=np.float32)

        # ref_imgs_info = {'image': cube_img.squeeze(0), 'mask': cube_mask.squeeze(0), 'depth': cube_depth.squeeze(0), 'pano_image': ref_imgs, 'pano_depth': ref_depths, 'pano_extrinsics': ref_c2w, 'intrinsics': cube_K.squeeze(0), 'extrinsics': cube_poses_c2w.squeeze(0), 'extrinsics_w2c': cube_poses_w2c.squeeze(0), 'near':near.repeat(6), 'far':far.repeat(6), 'index':ref_ids, 'scale':scale}
        ref_imgs_info = {'image': cube_img, 'mask': cube_mask, 'depth': cube_depth, 'pano_image': ref_imgs, 'pano_depth': ref_depths, 'pano_mask':pano_mask, 'pano_extrinsics': ref_c2w, 'intrinsics': cube_K, 'extrinsics': cube_poses_c2w, 'extrinsics_w2c': cube_poses_w2c, 'near':near.repeat(6), 'far':far.repeat(6), 'index':ref_ids, 'scale':scale}

    else:
        ref_imgs_info = {'image': ref_imgs, 'extrinsics': ref_c2w, 'extrinsics_w2c': ref_w2c, 'near':near, 'far':far, 'index':ref_ids, 'scale':scale, 'mask':pano_mask}
