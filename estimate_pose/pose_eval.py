...
def rotation_angle(rot_gt, rot_pred, batch_size=None):
    ...
    if isinstance(rot_gt, np.ndarray):
        rot_gt = torch.from_numpy(rot_gt).to(torch.float)
    if isinstance(rot_pred, np.ndarray):
        rot_pred = torch.from_numpy(rot_pred).to(torch.float)
...
def translation_angle(tvec_gt, tvec_pred, batch_size=None):
    ...
    if isinstance(tvec_gt, np.ndarray):
        tvec_gt = torch.from_numpy(tvec_gt).to(torch.float)
    if isinstance(tvec_pred, np.ndarray):
        tvec_pred = torch.from_numpy(tvec_pred).to(torch.float)
